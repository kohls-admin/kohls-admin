			...
			elseif c ~= "EOZ" then -- will raise an error next loop
				...
				elseif c == "u" then -- UTF8 string literal
					...
					while true do
						...
						elseif string.match(c, "%x") then
							...
							self:lexerror(
								string.format(
									"Invalid unicode character sequence. Expected alphanumeric character, got %s. Did you forget to close the code sequence with a curly bracket?",
									c
								),
								"TK_STRING"
							)
					...
					if not tonumber(unicodeCharacter, 16) or not utf8.char(tonumber(unicodeCharacter, 16)) then
						self:lexerror(
							string.format(
								"Invalid UTF8 char %s. Expected a valid UTF8 character code",
								unicodeCharacter
							),
							"TK_STRING"
						)
				...
				elseif string.lower(c) == "x" then -- Hex numeral literal
					...
					if not string.match(string.upper(hexNum), "%x") then
						self:lexerror(
							string.format("Invalid hex string literal. Expected valid string literal, got %s", hexNum),
							"TK_STRING"
						)
					else
...
function luaX:llex(ls, Token)
	...
	while true do
		...
		elseif c == "-" then
			...
			if c == "=" then
				self:nextc(ls)
				return "TK_ASSIGN_SUB" -- Luau Compound
			elseif c ~= "-" then
				return "-"
			end
			-- else is a comment

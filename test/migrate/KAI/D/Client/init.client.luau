...
Credit.Toggle.Activated:Connect(ToggleCredit)
cm.Close.Activated:Connect(ToggleCredit)
...
cm.Get.Activated:Connect(function()
	s.Market:PromptPurchase(plr, 172732271)
end)
for i, v in { { 129760409821083, 5 }, { 77680874300094, 45 }, { 130880915606069, 795 } } do
	local obj = cm["D" .. i]
	local success, info = pcall(s.Market.GetProductInfo, s.Market, v[1], Enum.InfoType.Asset)
	obj.Text = (success and info.PriceInRobux or v[2]) .. " R$"
	obj.Activated:Connect(function()
		lastAttemptedPurchase = v[1]
		server:FireServer(f.uID .. "KBulkPurchase", v[1])
	end)
end
s.Market.PromptBulkPurchaseFinished:Connect(function(player, status, results)
	if player ~= plr or (results.Items[1].id ~= 130880915606069) then
		return
	end
	server:FireServer(f.uID .. "KVIPCheck")
end)
cm.Hat.Activated:Connect(function()
	server:FireServer(f.uID .. "KHat")
end)
cm.Trail.Activated:Connect(function()
	server:FireServer(f.uID .. "KTrail")
end)
cm.Shine.Activated:Connect(function()
	server:FireServer(f.uID .. "KShine")
end)
cm.Join.Activated:Connect(function()
	local ok, result = pcall(s.Group.PromptJoinAsync, s.Group, 3403354)
	if ok then
...
task.spawn(function()
	local ok, result = pcall(plr.IsInGroup, plr, 3403354)
...
Direct.Close.Activated:Connect(function()
...
function f.HookList(a, b, c, d)
	...
	local FadeB = false
	b = a.Hide.Activated:Connect(function()
		if not FadeB then
			FadeB = true
	...
	c = a.Close.Activated:Connect(function()
		b:disconnect()
		c:disconnect()
		f.Hide(a, true)
	end)
...
f.HookToggle = function(a, b)
	a.Activated:Connect(function(c)
...
function ClientEvent(a, b)
	...
	elseif a == "KCmds" then
		...
		S.Changed:connect(function(p)
			if p == "Text" then
				SetList(f.Search(list, S.Text, function(a, b, c)
					for i, v in a[1] do
	...
	elseif a == "KVote" then
		...
		c1 = v.Y.Activated:Connect(function()
			cleanup(true)
		end)
		c2 = v.N.Activated:Connect(function()
			cleanup(false)
		end)
